...
% waypoints(:, 1)= waypoints(:, 1) + buildOffsetX;
% waypoints(:, 2)= waypoints(:, 2) + buildOffsetY; 
...
obs(:, 1)= obs(:, 1) + buildOffsetX;
obs(:, 2)= obs(:, 2) + buildOffsetY; 
...
dt = 0.01;                      % Pas de la simulation 
time = 0:dt:T_sim;

% NOUVEAU
rayon_obstacle_camera = 0.3;     % Rayon estimé en m de l'obstacle vu par la caméra
...
Ts_mpc = 0.1; % Horloge lente pour le contrôleur
...
%nlobj.MV(1).Min = -0.5; % v min ( si on met une valeur négative, on
%autorise la marche arrière)
...
%nlobj.Weights.ManipulatedVariables = [0.5, 1]; 
nlobj.Weights.ManipulatedVariables = [0.1, 0.11];
